...
export interface IKuaishouFetcher {
  ...
  ) => Promise<Result<ConditionalReturnType<KuaishouReturnTypeMap['单个视频作品数据'], M>>>
  ...
  fetchWorkComments: <M extends TypeMode = 'loose'>(
    ...
    cookie?: string,
    requestConfig?: RequestConfig
  ) => Promise<Result<ConditionalReturnType<KuaishouReturnTypeMap['评论数据'], M>>>
  ...
  fetchEmojiList: <M extends TypeMode = 'loose'>(
    ...
    cookie?: string,
    requestConfig?: RequestConfig
  ) => Promise<Result<ConditionalReturnType<KuaishouReturnTypeMap['Emoji数据'], M>>>
}
...
export interface IBoundKuaishouFetcher {
  /** 获取快手视频作品数据 */
  fetchVideoWork: {
    (options: KuaishouVideoWorkOptions & { typeMode: 'strict' }): Promise<Result<KuaishouReturnTypeMap['单个视频作品数据']>>
    (options: KuaishouVideoWorkOptions): Promise<Result<any>>
  }

  /** 获取快手作品评论数据 */
  fetchWorkComments: {
    (options: KuaishouCommentsOptions & { typeMode: 'strict' }): Promise<Result<KuaishouReturnTypeMap['评论数据']>>
    (options: KuaishouCommentsOptions): Promise<Result<any>>
  }

  /** 获取快手表情列表 */
  fetchEmojiList: {
    (options: { typeMode: 'strict' }): Promise<Result<KuaishouReturnTypeMap['Emoji数据']>>
