import { BilibiliDataOptionsMapKeys } from 'amagi/types'
...
class BiLiBiLiAPI {
  ...
  视频详细信息 (data: BilibiliDataOptionsMapKeys['VideoInfoParams']) {
    return `https://api.bilibili.com/x/web-interface/view?${data.id_type === 'bvid' ? 'bvid=' + data.id : 'aid=' + data.id}`
  }

  视频流信息 (data: BilibiliDataOptionsMapKeys['VideoStreamParams']) {
  ...
  评论区明细 (data: BilibiliDataOptionsMapKeys['CommentParams']) {
    return `https://api.bilibili.com/x/v2/reply?sort=1&ps=${data.number || 20}&type=${data.type}&oid=${data.oid}&pn=1`
  }
  ...
  二维码状态 (data: BilibiliDataOptionsMapKeys['QrcodeParams']) {
