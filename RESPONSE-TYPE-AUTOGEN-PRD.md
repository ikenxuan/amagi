...
## 2026-09-06 版面修正（只动控制台的形状，数据流一个字不改）

这一节推翻的是 **4.1 那张版面图与 4.2 那五块面板的排法**，以及 5.4 那张组件表里跟着它们走的
几条。上一节（数据流）与第五节（合并算法）一个字都不受影响 —— 接口、契约、样本落点全没动。

**被推翻的判断：「请求区在上、结果区在下，每份结果一张卡片」。** 它实现出来之后暴露三个问题，
成因是同一个 —— 那张版面图把界面当成一份**文档**在排，而它是一个**工作台**：

1. **纵向永远不够，横向几乎全空着。** 一张 `Card` 里最宽的东西是一段说明文字；同时响应 JSON
   与类型 diff 被压在两个 `max-h-96` 的框里上下排。而人要回答的问题是「这段 JSON 对应的类型
   对不对」—— 那个问题在上下两块之间来回滚是答不了的。
2. **滚不到底。** 结果区里每份结果一张 `OutcomeCard`（判定条 + 脱敏清单 + 两页 Tabs +
   四颗按钮 + 一张折叠表单），批量录 24 组之后那一列有几十屏高，而人一次只看一份。
3. **界面上少了最直接的那个答案。** 五块面板里有 diff（产物会怎么变）、有已提交的产物、
   有两组参数的对比，却没有「**刚打回来的这段 JSON，类型是什么**」。那是这个工具最常被问的
   一句话，而它原先只能从 diff 里反推。

修正后的形状：

| | 原方案（4.1 / 4.2） | 现方案 |
|---|---|---|
| 主版面 | 左栏 + 右栏，右栏上下两块 `Card` | **三栏并排**：请求 / 响应 / 类型，左边一条端点导航 |
| 滚动 | 页面滚 | **每栏自己滚、页面不滚**（`lg` 以上；判据在 `src/lib/pane.ts`） |
| 一次看几份结果 | 全部，竖着堆 | **一份**；「哪一份」由左栏底下的「最近」一行一条地选 |
| 「这份响应的类型」 | 没有 | **「类型」栏第一页**（`RecordOutcome.typeSource`，`server/declare.ts`） |
| 判定 / 脱敏 | 两块 `Alert` + 两枚 Chip + 一份清单 | 一枚 Chip 加一行，理由进 tooltip |
| 那五块面板 | 竖着排在结果区里 | 退到各自那一栏的 tab（集合 / 已提交 / diff / 对比） |
| HTTP 收据 | 没有 | `200 · 312 ms · 9.7 KB`（新增 `RecordOutcome.http`） |

**一个动作都没有被删掉**（留下 / 丢掉 / 批量 / 生成 / 对比 / 集合），删掉的是**重复**：同一件事
原先在按钮的 tooltip、一整块 `Alert`、和左栏那行标签上各写了一遍。留下什么的判据是
**「会改变下一步动作的话留在版面上，解释性的进 tooltip」**。

`Tabs` 顺带让懒加载那条边界真的生效：它只渲选中的那一页，于是没点开的 tab 连 chunk 请求都不发
（`Table` 一个就 104 KB）。实测入口 639,607 → 635,374 字节，产物合计 789,164 → 784,017。

5.4 那张组件表因此有三条作废：给 diff 点名的 `Pagination`（页码在一份 diff 里不指任何东西，
出口是按文件分组 + 每次多放一批，理由在 `Result.tsx` 的 `DiffPanel` 上）、给分组头点名的
`Meter`（覆盖率那条整个删了 —— 一次铺 61 行的清单里每行多一件装饰就是 61 份噪音）、
以及 `InputGroup`（这个界面没有 URL 栏，参数走 schema 派生的表单）。
...
| 项 | 数字 |
|---|---|
| 响应类型文件数 | **160**（其中 `index.ts` barrel **86**）—— 原写 150 / 78，是 2026-09-04 那几轮改名与合并之前的数 |
| 响应类型总行数 | **26,258**（`types/` 全目录 27,103 行的 97%）。这两个数由 `pnpm types:size` 量，别在文档里手抄 |
| 按平台分 | Bilibili 81 文件 / 9,870 行 · Douyin 44 / 13,952 · Kuaishou 19 / 1,824 · Xiaohongshu 15 / 607 |
| `_V0` 文件 | **72** |
| `_V1` 文件 | **0**（原有 2 个都不是真变体，2026-09-04 合掉了，见 1.3） |
| 不符合命名约定的文件 | **0**（原先 8 个，2026-09-04 清完，见阶段 4；2 个刻意豁免的只补了文件头说明） |
| 端点数 / 映射表键数 | **61** / 65（多的 4 个是抖音 passport 会话方法在借用这张表） |
| 映射表里是 `any` 的键 | **0**（原先 3 个，2026-09-04 填成真类型，见阶段 4） |
| 最大单文件 | `Douyin/UserLiveVideos/UserLiveVideos_V0.ts` 2,315 行 |
| 产物 `.d.ts` 体积 | `dist/index-*.d.ts` **825,177 字节**（806 KB，已含接进来的生成树） |
| 仓库里的真实响应样本 | **0**（这条**现在也依然是 0**，而且是有意的 —— 样本留本地） |
| 抓包溯源元数据（时间 / 参数 / 账号态） | **手写树 0；生成树每个文件都有**（溯源块，见下） |

> 上表 2026-09-05 重新量过一遍。原来那一版的数字是 2026-09-04 之前的现状，
> 而那之后手写树被动过三轮（合并假变体 `_V1`、8 个文件改名、3 个 `any` 填成真类型），
> 于是「现状盘点」这一节反而成了最先过期的一节。**行数一律按 `pnpm types:size` 的口径**
> （按 `\n` 切，每个文件多算一行 —— 换口径会与体积门禁的预算对不上）。
> **2026-09-10：`pnpm types:size` 已随体积门禁一起删掉**（判定：类型文件可以无限大）。
> 上面那句「按 `pnpm types:size` 的口径」现在只剩口径本身还成立 —— 现量一次即可，别把数字抄进新文档。
...
`packages/core/test/types/response-mapping.test-d.ts` 已经在逐个端点断言
...
比 61 个端点多 4 个 —— 多的正是抖音那几个 passport 会话方法在借用这张表。
...
**2026-09-05 补一条：「换成形状树同构」这句话还不足以定案，而卡住它的正是样本。**
顺着这条往下推会撞上一个岔路口，两条路都能自圆其说，而选哪条**只有真样本分得出来**：

- **严格占位符相等**（深层与浅层的键集合、可选性逐个相同，只有递归位置那一个键不比）：
  完全不会说谎，但在真实转发动态上**不会命中** —— 1.3 节实测过外层与 `orig` 侧确实有
  真差异（`editable` / `decoration_card` / `topic` / `RichTextNode` 那几条），
  不是只差嵌套深度。于是这条判据在它本来要解决的那个形状上等于没开。
- **放宽成「深层可以少掉浅层的可选键」**：能收成一个自引用类型，
  但它就是 1.3 节末尾已经栽过一次的那个错 —— 把 `orig` 侧与转发者侧合成一个类型、
  `Fan` 的键全变可选，下游按必需键读 `fan.color` 当场编译红。

还有一条不在原文里、实现时才会撞上的约束：**复用时必须借用被复用者的注释作用域**
（`objectBody` 里的 `scope = keyOf(node)`）。按自己的 key 走一遍 `claimDocsOnly`，
深层那些位置的 sidecar 注释会被标成 consumed 却没有任何地方渲染它 ——
静默丢注释，正是 `claimDocsOnly` 存在的理由所要防的事。

结论：**不要拿三层合成样本先把判据写死**（合成样本两条路都过，真样本才分得出对错）。
这三段已经写进 `render.ts` 的 `objectExpr` 注释，阶段 5 从这里接着做。
...
- [x] ~~选 `douyin.videoWork` 做样板~~ → **换成 B站 `comments`**，理由是种子：
...
- [x] **按形状截断数组**（这条原来没在清单里，第一次实录才发现必须做）
      ...
      **截断前后 `generateTypes` 的产物逐字节相同** —— 不是「大概不影响」，是钉死的。
      ...
      → **2026-09-05：那条「钉死的性质」其实能被证伪，已修。** 形状签名只描述结构、不含字面量值，
      而 `render.ts` 在命中 `literalPaths` 的位置上把取值收窄成字面量联合 ——
      于是 `{"list":[{"type":"T0"},…,{"type":"T4"}]}` 配 `literalPaths: [/list\[\]\.type$/]`，
      截断前渲染成五元联合、截断后只剩前三个。修法是让签名在**命中收窄的位置**把取值也算进去
      （与「超界整数单独算一种」是同一个思路），新增 `TrimOptions.literalPaths`、
      默认 `[]` 所以两个现有调用方一行不用改。
      两件事值得记：**① 命中的位置留全部取值，不照抄 merge 那边 `maxLiterals` 的放弃阈值** ——
      照抄会把截断的正确性绑在生成侧的另一个默认值上，调用方把 `maxLiterals` 调大就在录制时
      悄悄破掉，而样本是截完才落盘的、事后补不回来。**② 已提交的 28 个文件其实没被咬到**：
      `emit.ts` 往 `literalPaths` 里塞的只有判别式路径，而带 `[]` 的判别式在那里被直接拒掉，
      所以收窄从来没落到数组里过 —— 这条性质是被那道闸兜着，不是 trim 自己成立。
      实测：真 corpus 上带不带 `literalPaths` 产出的字节数完全相同；把 `comments` 回填到
      100 条 replies 时两种口径都从 1,302 KB 掉到 260 KB
      → 同一轮顺手修掉一个**确定性**缺陷：`matchesLiteralPath` 没清 `lastIndex`。
      带 `g` / `y` 标志的 `RegExp` 的 `test()` 是有状态的，实测 `/\.type$/g` 问过 `a.type`
      之后再问 `b.type` 会返回 **false**（同一条路径连问两次同样是 true / false 交替）。
      在此之前只有 merge 会调它，这次 trim 也开始调，于是同一份 pattern 数组在整棵树上
      被问成千上万次 —— 答案跟着遍历顺序变，而「同一批样本进去、产物逐字节相同」是这套
      东西的全部前提。`scrub.ts` 那侧的 `test()` 早就为同一个理由清过零，漏的是这一处
      → **2026-09-07：这一步在展示侧曾是无声截断的第三处** —— 界面上只看得到截后的样本，
      而「emoji_list 只有 3 条、真实响应有几百条」无人解释。披露的字段（`rawPayload` /
      `payloadTrimmed`）与「原始 / 样本」切换记在阶段 3，**入库样本的截断本身照旧**
...
- [x] 「一键补样本」：对当前端点按参数矩阵批量录一轮
      ...
      **批量录制不等于批量入库**：每组都只到待定为止，要不要留还是人一组一组看 ——
      这正是这个工具存在的理由
      → **2026-09-05 修掉一个把这条几乎作废的 bug**：每组的 `shapeChanged` 与 diff
      原先只跟**磁盘上**的样本比，而同一批前面几组只进了内存、一份都没落盘。
      于是一个 0 样本的端点跑 6 组同形样本，6 份全被报成「带来了新形状」，
      人照着提示把 6 份全留下 —— 那正是本文档拿「两份 2.57 MB 的重复 B站 `comments`」
      当反面例子要消灭的那件事，工具反而在制造它。现在前面几组的样本会喂给后面几组当基线
      （只收 `ok` 的那些，被判定拒掉的风控页不参与类型）。
      循环本体抽成 `server/batch.ts` 并补 13 条测试 —— 它原先长在 `handle` 里、
      一条测试都没有，而「后面几组看得见前面几组」「一组炸了不带崩整批」
      「最后一组之后不睡」三件都是被文档承诺过、却没人钉住的行为
...
- [x] **回环下只认回环 `Host`，写接口只接受同源 `Origin`**（这条原来没在清单里）
      ...
      → **2026-09-05：这两道闸各有一个洞，都修了，并抽成纯函数补了 20 条测试
      （`server/guard.ts`，切法照待决 #9 那条 —— 判定逻辑可测，路由层仍然不测）。**
      - **`Origin` 闸是按主机名比的，而且全程不校验 `Content-Type`。** 主机名比不了端口
        （也不该比 —— 合法前端在 Vite 的 5173，本来就与 API 端口不同源），于是本机
        **任何一个** `http://localhost:<别的端口>` 的页面都能过这道闸；而 `text/plain`
        是 CORS 安全列表值、**不触发预检**，请求带着你的 cookie 真的打出去，
        `corpus/` 与 `packages/response-types/` 真的被写。补第三道闸：
        **写接口只收 `content-type: application/json`**，别的 415 —— JSON 不在安全列表里，
        跨源请求必须先预检，而这个服务从不回 CORS 头，浏览器自己就拦了。
        代码里那句「`<form>` 跨站 POST 不带 `content-type: application/json`，所以那条路
        也走不通」原本就断言了这个保护，只是从来没实现。
      - **绑局域网时这道闸让整条流程不可用。** `Origin` 闸无条件生效、白名单又硬编码成
        回环名，所以按 README 那条 `--host 0.0.0.0 --token …` 起服务、从另一台机器打开界面时
        **每个写请求都 403**（Vite 的 `changeOrigin` 只改 `Host`，不改 `Origin`）。
        本节上面写的是「只接受**同源**」，代码实现的是「只接受**回环源**」，两者恰好在这里背离。
        修法：**有口令时 `Origin` 闸让路** —— 口令不是 cookie，浏览器不会替攻击者页面带上它，
        那道闸严格更强。
      顺带把「是不是回环」那份三项比较从两处（启动检查、`Host` 闸）合成一处：
      两处哪天不一致，本身就是一个洞（一边认为不用口令、一边认为不用查 Host）
...
- [x] **就地生成**：加一个「生成这个端点的类型」的动作。
      ...
      **它必须跳过根 barrel 与平台 barrel**（`isEndpointOwnedFile`）——
      ...
      回给前端的话里也这么说
      → **2026-09-05 修了两处，都是「说过要做但没做到」**：
      **① 它只写不删。** 补一份样本让判别式忽然可发现之后，布局会从
      `Comments/Comments_V0.ts` + `Comments/index.ts` 翻成 `Comments/guards.ts` +
      `Comments/<取值>/…` —— 旧的两个文件没人删，而平台 barrel 按判据被排除、不重写，
      于是它仍然 `export type { Comments_V0 } from './Comments'`，指向的旧文件还在。
      **`tsc` 全绿，下游拿到的还是旧类型，新写出来的联合谁也 import 不到。**
      现在先写再删（写到一半失败时树只会多东西、不会少东西），清理范围严格限制在
      `<平台>/<Endpoint>/` 底下、只删 `.ts`、路径至少三段 —— 于是两层 barrel 在判据上就碰不到。
      端点目录从产物路径本身推出来，不重算一遍 `plan.ts` 的命名规则。
      **② 「barrel 完整性只有全量 gen:types 能保证」这句话其实没回给前端。**
      它写在前端的 `warnings.length > 0 ? … : …` 的 else 分支里，于是样本超 90 天、
      注释孤立这类很常见的告警一出现就把它顶掉；curl 用户则从来没见过它。
      现在它是契约里的 `GenerateResult.note`，每次都回、界面无条件显示。
      顺带 `summary` 原先从头到尾没人显示 —— 于是 3 份样本全被判定拒掉时界面说
      「没有产出文件（这个端点还没有可用样本）」，与旁边「本地已有 3 份样本」自相矛盾，
      而真实原因就躺在被丢掉的 `summary` 里
...
- [x] **`trimSample` 那处无声截断补上披露**（2026-09-07，这条原来没在清单里）
      → 与 diff 一批 400 条、高亮 20,000 字是同一类事、同一类修法：**上限不是问题，
      无声才是**。录制链路上 `server/outcome.ts` 对 raw / normalized 先过一遍 `trimSample`
      （每个数组留前 3 条 + 形状没见过的条目），截出来的值当作 `payload` 回给前端，
      裁剪记录（`{path, from, to}`）直接丢弃 —— 于是「响应」页显示的 `emoji_list`
      只有 3 条，而真实响应有几百条，界面上没有任何一句话说这件事。修法五件一起落：
      ① 契约加 `rawPayload`（裁剪 + 脱敏之前的原始响应 —— 不脱敏是过的：看这个控制台的
      人就是提供 cookie 的那个人，要落盘的样本照旧走脱敏的路）与 `payloadTrimmed`
      （**跟着 payload 那一层走**，payload 用 normalized 就用 normalized 的记录 ——
      Chip 说的话必须对展示物为真）；② 「响应」页默认显示原始、可切样本
      （`ToggleButtonGroup`，当初组件表里给 Pretty/Raw 点的正是它），`rawPayload`
      缺失（旧 server / `compute` / 一发都没打出去）时回落样本视图、切换整个不渲；
      ③ 动作条加「已截断」Chip（一处说全 `emoji_list 371→3`、多处报处数，明细进 tooltip）；
      ④ 收据 `http.bytes` 改为**真实响应体积**（原先数的是脱敏后样本，一份 280 KB 的
      响应在收据上只报 4 KB）、样本体积另立 `http.sampleBytes`，标题行两个并排报；
      ⑤ 复制按钮跟着当前视图走，标签说清复制的是「原始」还是「样本（裁剪 + 脱敏后）」。
      **入库样本维持「先裁剪再脱敏」不变**（`trim.ts` 与 `record-corpus.mts` 一律不动）：
      corpus 的体积纪律与展示的披露纪律是两件事，前者靠截断保住，后者靠这次加的字段。
      顺带把「那两处」改成「三处」：`Result.tsx` / `server/highlight.ts` /
      `CodeBlock.tsx` / `contract.ts` 里注释引用的正是这条纪律
...
- [x] ~~逐平台推进~~ → **不做**。等某个端点的样本覆盖到「人愿意为它背书」的程度再一个一个换
...
- [x] ~~迁移一个端点就把旧的 `_V0.ts` 删掉，不留两份~~ → **改成有意识地并存**。
...
- [ ] 递归判别联合生成，验证与现有 `Dynamic/` 6 个变体的对齐情况
      ...
      **不会无限展开**（`objectExpr` 先登记名字再填 body），所以这不是个崩溃风险，
      ...
      → **2026-09-05：那三条 `NOT_IMPLEMENTED` 里只有第一条真的带了「怎么验」**
      （`report.ts` 的文档注释第 61 行承诺「每条都写清」，实际第二、三条都没写），
      已补齐。同一轮把「判据换成形状树同构」这句话往下推了一步，发现它还不足以定案 ——
      两条候选判据一条在真实转发动态上不命中、另一条会重演 1.3 里那个下游编译红的错，
      **选哪条只有真样本分得出来**。推导写在 5.2 节末尾与 `render.ts` 的 `objectExpr` 注释里
...
- [x] `types:check` 进 CI（与 `openapi:check` 同级）—— 等生成器的落盘那层做完。
      **体积门禁那条已经进了**：`.github/workflows/release.yml` 的质量门禁 job 里
      ...
      （读不了的样本、路径撞车、残留文件）
      → **2026-09-05 更正这一句：CI 上它做不到产物树自检。** 那三样全都要有样本才碰得到，
      而残留文件检测在 `gen-types.mts` 的 `else if (check)` 分支里 —— 零样本时走的是它前面那条
      短路（`gen-types.mts:144`），CI 上永远到不了。CI 上这一步实际是个**冒烟测试**：
      验落盘那一层端到端跑得通、已提交的产物树读得开，坏了会红。没有假绿灯
      （它明说跳过比对），但「产物树自检」这个说法名不副实，`release.yml` 的注释已经改准
      → **2026-09-10 补上「树自洽」这一条**：零样本分支现在会验两个东西 —— **barrel 与产物树
      是否一致**（`reconcileBarrels`），以及有没有**端点目录缺 `index.ts`**（那种目录既进不了
      barrel 又不会报错，是从发布面上悄悄消失的那一类）。不一致就置 1。
      它补的正是上面那段当时没解决的半个洞：「barrel 只有全量生成会写」等于没有常驻写入方，
      而那次没人跑 —— 根 barrel 在零样本状态停了一整轮、包对外导出 0 个类型，四处门禁全绿。
      同一个函数也是控制台「生成类型」写完端点后重算两层 barrel 的那一份
      （`storage.writeGeneratedBarrels`）—— **一份规则，两个写入方**，这是它成立的根据
...
- [x] 类型体积门禁：总行数 / 单文件行数上限，超了要报（防止全自动把 26k 行变成 10 万行）
      ...
      这一点要写在预算旁边否则下一个人会调错：
      - **手写树是棘轮**（实测 26,258 行 / 160 文件，预算 28,900 / 176 / 单文件 2,500）——
        它只该不变或变小（被生成产物逐个替换掉），余量刻意小。
        **树变小之后预算必须跟着往回拧**，2026-09-05 就是漏了这一步才发现的：
        预算最初按合并那 2 个假变体 `_V1` 之前的 27,249 行 / 162 文件设，合完树掉到 26,258，
        预算还留在 29,500 / 200 —— 余量从 +8% 悄悄变成 +12%，等于凭空多出约 1,200 行
        可以无声无息加回来的额度。棘轮不往回拧就不是棘轮。这笔账已写进脚本注释。
      - **生成树预期会长**（实测 2,475 行 / 28 文件，端点覆盖 12/61）—— 补样本必然让总行数涨，
      ...
      → **2026-09-10 整条删除：判定「没有用」，不再重建。** 判据是**类型文件可以无限大** ——
      体积不是这类产物该拦的判据，这道门禁产出的只有「为了过门禁去调预算」的噪音。
      拆掉的接线：`packages/core/scripts/check-type-size.mts` 脚本、根与 core 的 `types:size`
      脚本项、`release.yml` 质量门禁里那一步、`docs/v7/dev/` 三处（命令表 ×2、PR 清单 ×1）。
      上面这一整段是它拆除前的最后状态，留着是为了「别照着再建一次」。
      它的最后一次失败正好说明问题：红的三个 douyin 单文件（1754 / 1233 / 1206 行）在同端点
      对比里**都比手写树小**（1754 vs 2031、`liveRoomInfo` 706 vs 2314），而 1,200 那个数是
      按 2026-09-05 那批 12 个端点定的 —— 数字早就与树脱节了。
...
- [x] corpus 提交策略写清楚（见「十一、待决」第 1 条的结论）→ 写进 `dev/contributing.mdx`
      ...
      （`types:size` 那条已于 2026-09-10 随门禁一起删掉，清单现为 10 项 —— 见阶段 6）
...
- **类型体积失控**。现在 26,258 行手写响应类型编译出来的 `dist/index-*.d.ts` 已经是
  **776 KB**（接生成树之后 824 KB，因为多了 28 个文件的声明），直接影响下游 `tsc` 速度。
  ...
  结构等价复用（5.2）和体积门禁（阶段 6）原本都是为这个准备的。**2026-09-10 门禁已删**
  （判定：类型文件可以无限大，体积不是该拦的判据），所以现在只剩结构等价复用这一道防线 ——
...
- **脱敏漏了而没人发现**。样本不进 git 降低了这条的后果（不再进公开历史），但没消除它：
  ...
  **`suspects` 现在会报「像人写的话」**（长度 ≥ 4 且含 CJK / 假名 / 谚文 / 西里尔）。
  ...
  → **2026-09-05 撞上第六条，而它不属于「按键名追不完」那一族 —— 前五道防线一条都没坏，
  是白名单自己把整块负载放过去了。** `scrub.ts` 的白名单命中就 `return value`、
  **连递归都不进**，而默认白名单里有一条按键名匹配 `type|kind|code|status|result`。
  B站搜索的 `data.result` 正是这个键名，而它是整块结果列表 ——
  实测那一整块的昵称、UID、带签名的 CDN URL 一个都没换，
  且 `replacements` / `suspects` / `leaks` **三个全空**，人连「这里可能有问题」都看不到。
  修法是把两种白名单分开：**默认白名单只保标量**（它保的是判别字段，而判别字段永远是标量），
  命中容器时让路继续往下钻；**调用方显式传的 `keep` 仍然整块保住** —— 那是人明确做的决定
  （有一条测试专门钉着这个能力）。
  容器让路时刻意**不报 warning**：那不是数据或配置出了错，是这条白名单本来就不该管容器，
  报了只会在常见形状上变成噪音。
  **本地 14 份样本逐份扫过：0 份含「白名单键名 + 容器值」这种形状**，
  所以这个洞从来没在现有样本上真的漏过东西，不用重录 —— 但下一轮录搜索类端点时它会第一次真触发。
  同一轮还修了两条：**① `CREDENTIAL_PARAM` 漏掉这个仓库自己在用的凭证名**
  （`SESSDATA` / `bili_jct` / `DedeUserID` / `buvid3` / `sid_guard` / `s_v_web_id` 六个一个都不命中，
  而 `sid_guard` 是第七节点名过的；其中两个只因为「32 位连续串」进了 suspects、值照样落进
  `metadata.params`）—— 直接违反第七节那条 cookie 禁令。补齐时写死一条反向约束：
  **绝不能加光秃秃的 `uid`**，那是正当业务参数，删了这份样本就说不清问的是谁。
  **② 一个 `__proto__` 键就能让生成器整个抛掉**：`render.ts` 的注释查表用 `docs[path]`，
  而路径来自样本键名 —— `JSON.parse('{"__proto__":1}')` 产出的是**真的自有键**，
  于是取到 `Object.prototype`、`!== undefined` 成立、接着当字符串用，
  `text.replace is not a function`。`constructor` / `toString` / `valueOf` 同样能崩，
  而且**连 sidecar 都不用有**。改成 `Object.hasOwn`。`merge.ts` 那侧用 `Map`、天生免疫，
  全仓就漏在这一处查表上。
...
9. ~~**`packages/web` 的两处形状。**~~ **两条都按倾向做了（2026-09-04）。**
   ...
   - **`server/` 的纯逻辑抽出来测了**（`server/outcome.ts` + 17 条），HTTP 路由层不测。
     ...
     → **2026-09-05 按同一条做法又抽了两块**，因为「路由层不测」被当成了「路由层里那些
     判断也不测」，而两个真 bug 恰好都长在那里：`server/guard.ts`（三道闸 + 口令，20 条）
     与 `server/batch.ts`（批量录制那个循环，13 条）。判据是**这段逻辑有没有分支** ——
     有分支就抽出来，剩下真正的路由层（`url.pathname === …` 那串分发）确实不值得测。
...
12. **2026-09-05 审计出来的四条，每条都要改语义阈值或改判定结果，所以列在这里而不是直接改。**
   同一轮里「一眼是 bug、修法没有取舍」的那些已经修掉了（见「十、风险」那条与阶段 3 各处），
   剩下这四条都带一个方向性的选择：

   - **`classifyResponse` 有四处能绕过**（`corpus.ts`）。其中**「业务码是字符串时整条判定失效」
     这一条没有取舍** —— 第五节规则表自己写着平台会给 `"12061"`，所以那是纯缺陷，
     该修（改成认数字形式的字符串）。要拍板的是另外三条：**① 只查顶层业务码**
     （快手的 `data.visionVideoDetail.result = 2` 判成 `store`，而模块头说要挡的就是它）——
     往下查几层？查深了会把正常的嵌套 `code` 字段当业务码。
     **② `isNullShell` 只看 `data` 的直接值**，一个非 null 兄弟键就绕过，而且结论是
     `confident: true`（它比 `false` 更糟：下游会信它）。**③ 调用方传的 verdict 无条件压过判定器**，
     连 confident 的 reject 都能压 —— 与 `CorpusVerdict.confident` 的文档说法相反。
   - **`pickDiscriminant` 的重复守卫太松**（`discriminant.ts`）。守卫是「至少有一个取值出现过
     一次以上」，于是 6 个变体各 1 份、其中一份重录一次（按 `bvid` 与 `aid` 两条种子拿到同一个
     作品就会这样），`data.id_str` 就能与真判别式同分并因为路径更浅而胜出，
     产出一堆按作品 ID 命名的目录。收紧的代价是**样本少时真判别式也检不出来** ——
     而「样本少」正是现在的常态（12 个端点大多 1 份）。所以这条的方向取决于
     「宁可漏检还是宁可误检」。
   - **对象的键从不脱敏**（`scrub.ts` 的 walk 只换值）。`{"photos":{"5312434312":{…}}}`
     只有 2 个键、够不上 `MAP_MIN_KEYS`，于是真 ID 既进属性名**又进类型名**，而产物是提交的。
     修它要造**同长度的假键**（否则形状变了，违反第七节「脱敏不能改变类型形状」），
     还要和映射表收索引签名那条判据对齐 —— 是设计活，不是补一条正则。
   - **残留检查不看数字**（`scrub.ts` 的 `leaks`）。同一个值写成字符串能抓到、写成 number 抓不到
     （`{"author":{"mid":X},"stat":{"owner":X}}` 里 `stat.owner` 原样留下且 `leaks: []`）。
     补上数字的代价是误报会**直接拦住入库**（残留非空时录制器不让入库），
     而数字撞车比字符串撞车常见得多。
