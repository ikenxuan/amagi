...
- [x] 全程盯 `test:types` 与下游 kkk 的 `typecheck`——类型变了下游立刻会红
      → 每轮都在跑，这一轮连着下游一起过了一遍完整的：
      amagi `test:types` **100 文件 / 2,063 条 / 0 类型错误**；
      重新 `tsdown` 构建 core（下游吃的是 `dist/*.d.ts` 而不是源码），
      再在 `D:\GitHub\karin-plugin-kkk` 跑 `tsc --noEmit`，**退出码 0**。
      这一轮值得单独验一次，因为往 `packages/core/src/types/generated/` 塞了一整棵新树
      （24 个文件），而那底下的东西会进 `dist` 的类型声明 —— 下游绿说明新增的产物
      既没有和手写树撞名，也没有把任何既有导出改窄
