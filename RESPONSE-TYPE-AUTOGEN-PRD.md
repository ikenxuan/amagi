...
**这一条没有被样本进 git 解决，也不该靠那个解决** —— 该解决它的是在**产物文件头里**
写上录制时间与参数（还没做，记在风险那一节）。
...
- **类型体积失控**。现在 27,087 行手写响应类型编译出来的 `dist/index-*.d.ts` 已经是
  ...
  **但要注意体积门禁现在量不到生成树**（它只量 `core/src/types/ReturnDataType`），见阶段 6 那条。
...
- **样本过期而无人知**。平台改了字段，但没人重新录，类型就一直是旧的。
  ...
  **样本不进 git 之后这条风险变大了**：过期与否只有本地那台机器知道，别人 clone 下来
  看到的是产物而看不到证据的年龄。缓解只有一条 —— 产物文件头里写上录制时间（还没做）。
...
- **脱敏漏了而没人发现**。样本不进 git 降低了这条的后果（不再进公开历史），但没消除它：
  脱敏产物仍然会喂给生成器，而**字段名会进产物**。最近三轮连着修的三类漏洞
  （残留子串、值形状优先于键名、字符串里套 JSON）说明按键名追是追不完的。
