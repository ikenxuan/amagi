...
- [ ] `MajorType` / `AdditionalType` 两个 enum 保持手写（放 `index.ts`），
      ...
      → **静态那半已经数出来了，结论比 §1.1 写的更糟**：不是「声明 17 个只建模了 6 种」，
      是那 27 个成员（17 + 10）被已建模的类型引用了 **0 次** —— 没有 `MajorType.ARCHIVE`
      这样的引用，也没有 `'MAJOR_TYPE_ARCHIVE'` 这样的字面量，两个 enum 在整个 `src/`
      里除了自己的声明文件之外没有任何 import。而它们本该描述的
      `modules.module_dynamic.major.type` 类型是裸 `string`。
      也就是说 **27 个带中文语义注释的成员是纯声明**：人的知识写在那儿，
      类型系统与调用方都拿不到 —— 这正是「手写语义与实测形状分居两处」的缩影。
      `test/types/dynamic-enum-coverage.test.ts` 把这个账记住了（4 条断言，数字一变就红），
      所以「缺口有多大」从没人知道变成一个被盯着的数。
      **删还是收窄仍然等样本**：收窄到 enum 会拒绝平台返回的新取值（与索引签名那条
      「加字段不算 breaking」的承诺冲突），删掉又会丢掉那 27 条中文语义。
      两条都得先知道线上实际出现哪些取值。
