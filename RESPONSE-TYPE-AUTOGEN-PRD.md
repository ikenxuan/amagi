...
| `_V0` 文件 | 62 |
| `_V1` 文件 | **0**（原有 2 个都不是真变体，2026-09-04 合掉了，见 1.3） |
...
**2026-09-04 手工合掉了**（不等生成器 —— 那是手写树自己的债，与替换节奏无关）。
两件事与上面那张表不一致，都是真做才看清的：

1. **上表只列了 5 条，实际是 12 条**；而 `DYNAMIC_TYPE_DRAW` 那一对**从没人列过**，
   它有 7 条差异、与 AV 只有 3 条重叠。也就是说 1.3 节原本低估了这两对的差异量。
2. **AV 的装扮卡刻意分成两个类型**（`DecorationCard` 走 `orig` 侧、
   `PurpleDecorationCard` 走转发者侧，后者的 `fan` 是 `{ [key: string]: any }`）。
   先合成一个、把 `Fan` 的键全拉成可选 —— 下游立刻红了（它按必需键读 `fan.color`）。
   分开写既是逐路径合并的忠实结果，也不削弱 `orig` 侧。

实效：手写树 162 文件 / 27,249 行 → **160 / 26,258（−991 行）**。
比 PRD 估的 −1,166 少一点，因为合并后要写清「为什么这些字段是可选的」——
文件头那两张差异表加上从旧 `_V1` 搬来的类型，约 180 行。方向仍然是变小、变准。

**下游一行都不用改**（一开始以为要改：`orig.…decoration_card` 变可选之后，
把它传进那个按 `DynamicTypeDraw` 的**必需**键声明参数的函数会报 TS2345。
但实测下游 `typecheck` 退出码 0 —— 它用的是非转发 `DynamicTypeDraw` 那条路径，
那边的 `decoration_card` 仍然必需）。
...
**2026-09-04 实测：这条只做到了一半，而缺的那一半正好是递归。**
结构等价复用（`render.ts` 的 `objectExpr` + `keyOf`）确实在跑，但它要求
「键集合与每个键的**类型**逐字相同」—— 而递归形状的每一层嵌套深度不同，
`orig?: Orig` 与 `orig?: Orig2` 不是同一个键类型，于是复用不命中。
实测三层同形样本产出 `Probe` / `Orig` / `Orig2` 三个类型而不是一个自引用类型。

**不会无限展开**（`objectExpr` 先登记名字、入队，再填 body），所以这不是崩溃风险。
真实后果是两条：类型比它该有的样子胖；而且**深度取决于恰好录到多深** ——
录到三层转发就产三层类型，录到五层就五层，同一个端点的产物会随样本变形。
所以这一条要等阶段 5（B站动态专项）真做时一起解，判据得从「类型逐字相同」
换成「形状树同构」（把递归位置先设成占位符再比）。
...
**第三条是后来补的，因为前两条不够**：绑在回环上**不等于别人碰不到**。
任何网页都能把自己控制的域名解析到 `127.0.0.1`（DNS rebinding），于是那个页面能用
本机 cookie 发请求、能让服务往 `corpus/` 与 `packages/response-types/` 写文件 ——
而这个服务的写接口在回环下是无鉴权的（口令只在绑局域网时才有）。
所以回环模式下只认回环 `Host`，写接口只接受同源或无 `Origin` 的请求。
干跑验过四种情况：正常 200、伪造 Host 403、跨站 POST 403、同源与无 Origin 仍然 200。
...
- [ ] ~~选 `douyin.videoWork` 做样板~~ → **换成 B站 `comments`**，理由是种子：
      抖音 19 个端点里只有两个 emoji 表免种子，`videoWork` 要 `aweme_id`；
      而 B站一个公开 `bvid` 就能顺依赖图长出 4 个端点的样本。
      「有大体量手写类型可比对」这个条件同样满足（`WorkComments_V0.ts` 1,928 行）
- [ ] **手工录 5~10 份真实响应（不同作品：视频 / 图集 / 长文 / 已删除 / 私密）**
      → **这条仍然没做，而且它是现在最大的一个洞。** 本地 14 份样本覆盖 12/61 个端点，
      大多只有 1 份，而 1.3 节那个结论（两个 `_V1` 是抓包漂移不是真变体）与 3.2 节
      「内容驱动的变体只能靠样本量」都指向同一件事：**缺的不是工具，是覆盖不同内容的样本**。
      工具链现在整条通了（控制台 → 录 → 看 diff → 留下 → 生成），
      所以这条从「要写代码」变成了「要有 cookie 坐下来录一轮」
...
- [x] **不要**把 cookie 回显到页面上
      ...
      一个都搜不到
      → cookie 配置抽屉落地后这条更要紧了（`/api/cookies` 现在会回状态）：
      它回的是「有没有、多长、从哪来」三样，**长度是刻意保留的** ——
      `sessionid=…` 少一截时看得出来，而值本身在页面上没有任何用途
- [x] **回环下只认回环 `Host`，写接口只接受同源 `Origin`**（这条原来没在清单里）
      → 前两条安全约定有个缺口：绑在 `127.0.0.1` 上**不等于别人碰不到**。
      任何网页都能把自己控制的域名解析到 127.0.0.1（DNS rebinding），浏览器认为同源，
      而请求真的打到本机这个服务上 —— 于是那个页面能用**你的 cookie** 发请求、
      能让服务往 `corpus/` 与 `packages/response-types/` 写文件。
      回环模式下口令是没有的（那条只在绑局域网时要求），所以写接口原先是无鉴权的。
      两道闸：回环下 `Host` 必须是 `127.0.0.1` / `localhost` / `[::1]`；
      `POST` 带了 `Origin` 且不同源时拒。**无 `Origin` 的放过** ——
      那是 curl 与本机脚本，而浏览器发跨站请求时一定带它。
      干跑验了四种：正常 200、伪造 Host 403、跨站 POST 403、同源与无 Origin 仍 200
...
- [ ] **显示并可改端点的参数定义**：显示端点源文件（便宜）；
      ...
      **改 zod 声明还没做** —— 那是另一个量级
- [x] **同形样本提示**：录到一份与已有样本同形的（那两份 B站 `comments` 就是），
      当场告诉人「这份不会改变类型」让他丢掉 —— 而不是自动去重（自动去重会让
      「我明明录到过」变成一件说不清的事）。见待决 #8
      → 做了，而且**判据不是「diff 是不是空的」**，那条会一直判错：产物文件头里有溯源块
      （几份样本、参数哈希、录制日期），所以多录一份样本必然让 diff 至少多两行注释 ——
      哪怕形状一模一样。于是 `RecordOutcome` 多一个 `shapeChanged` 字段，由 server 算：
      只数**形状行**，`//` 与 JSDoc 的 `*` 一律不算（sidecar 注入的注释同理，
      它描述语义不描述形状）。
      界面上三处：结果卡片顶一条「这份没带来新形状，建议丢掉」的告警、
      「留下」按钮降成次要样式（不禁用 —— 换掉某份风控页样本是正当理由）、
      队头直接报「N 份可入库，其中 M 份没带来新形状」，不用一张张卡片翻。
      测试三条钉住它：同形时 `shapeChanged: false` **而 diff 非空且全是注释行**、
      新字段时 `true`、第一份样本也是 `true`
...
- [x] **合并掉那 2 个假变体 `_V1`**（见 1.3），它们应该塌成一个带可选字段的类型
      （这条与迁移解耦：它是手写树自己的债，不等生成产物替换）
      → 合了，两对（AV + DRAW）。详情与两处「真做才看清」的事写在 1.3 节末尾：
      差异实际是 12 + 7 条（原表只列了 5 条、DRAW 那对从没人列过）；
      AV 的装扮卡**刻意分成两个类型**而不是合成一个（合成会让 `Fan` 的键全变可选，
      下游按必需键读 `fan.color` 会红）。
      手写树 27,249 → **26,258 行**（−991）。对外类型名一个都没变，下游 typecheck 0 错。
      合并后的文件头写清了「再抓到形状不一样的报文直接改这个文件，不要再开 `_V1`」——
      这条比合并本身更要紧：`_Vn` 的语义是「合不掉的形状序号」，
      而「两次抓包不一致就再开一个文件」是它被误用的样子
...
- [ ] 递归判别联合生成，验证与现有 `Dynamic/` 6 个变体的对齐情况
      （那 2 个 `_V1` 已经在阶段 4 手工合掉了，见 1.3）
      → **实测了现状，结论是「不会炸但也没收成自引用」**（2026-09-04）：
      结构等价复用（5.2）要求「键集合与每个键的类型逐字相同」，而递归形状的每一层
      嵌套深度不同 —— `orig?: Orig` 与 `orig?: Orig2` 不是同一个键类型，于是复用不命中，
      产物逐层展开成 `Orig` / `Orig2` / `Orig3`，深度由样本里最深那一层决定。
      **不会无限展开**（`objectExpr` 先登记名字再填 body），所以这不是个崩溃风险，
      而是「类型比它该有的样子胖，而且深度取决于恰好录到多深」。
      已把这条写进 `report.ts` 的 `NOT_IMPLEMENTED`（带「怎么验它还成不成立」），
      并加一条测试钉住当前行为 —— 真收成自引用之后那条测试会红，那时该删掉清单里那一条
...
- [x] **上面那节重写了（2026-09-04 结论反了）**。`dev/contributing.mdx` 那节标题从
      「响应样本要提交进 git」改成「响应样本留在本地，不进 git」，三条推翻理由逐条写清
      （伪命题、体积估错一个数量级、脱敏一直在漏）；同一节补了「本地那份是重新生成的输入」
      与产物文件头里的溯源块。`dev/add-api.mdx` 里「放进 `test/fixtures/<平台>/`」的指引
      改成走 Web 控制台（历史做法那段保留 —— 它解释了 corpus 为什么存在）。
      PR 清单里 `types:check` 那条注明「CI 上它只做产物树自检」。
      顺手修掉同一页里与新结论冲突的部分（不改这页会自相矛盾）：命令表以控制台两条领头、
      `record:corpus` 标成辅助、产物路径从 `packages/core/src/types/generated/`（已经错了）
      改成 `packages/response-types/src/generated/`
- [x] **`packages/web` 的 README**：两个进程怎么一起跑、绑局域网要给口令、
      cookie 从哪读、包里三层各自的职责与**为什么必须是三层**、样本落在哪且别清掉它、
      前端产物不提交
...
- **类型体积失控**。现在 26,258 行手写响应类型编译出来的 `dist/index-*.d.ts` 已经是
  ...
  **但 5.2 只做到了一半**（见那一节的实测）：递归形状的复用不命中，于是转发动态那类
  自引用结构会逐层展开，深度还取决于恰好录到多深。B站动态专项要先解这个 ——
  否则「一个端点吃掉 20% 类型代码」那件事会在生成树上重演。
...
10. **`packages/web` 剩下一条要不要做：改端点的 zod 声明。**
   往 TS 源码里写代码，走 AST 窄口子、落在 `packages/codemod`。这是「web 全权管理增删改查」
   里的「改」，但它与前面那些不是同一个量级 —— 前面都是读注册表与算产物，这条要改**源码**。
   倾向：等确实频繁撞到「为了加个参数要开编辑器」这个疼点再做。
   （同形样本提示那条已经做了，见阶段 3。）

11. **`corpus/` 现在最缺的不是工具，是样本。** 阶段 0 那条「手工录 5~10 份不同内容的响应」
   至今没做，而它现在是最大的一个洞：本地 14 份覆盖 12/61 个端点、大多只有 1 份，
   而 1.3 节（两个 `_V1` 是抓包漂移）与 3.2 节（内容驱动的变体只能靠样本量）
   都指向同一件事。工具链整条通了之后，这条从「要写代码」变成了
   **「要有 cookie 坐下来录一轮」** —— 那是我做不了的部分。
   录之前值得先做的一件事：脱敏刚补了第五道防线（见「十、风险」那条），
   在新样本上它会第一次真正被用到，所以第一轮录完要**逐条看 `suspects`**。
