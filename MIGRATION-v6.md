...
3. **HTTP 接口规范化** - 采用 RESTful 风格路由
4. **类型系统增强** - 支持 `typeMode: 'strict'` 获取严格类型
5. **Fetcher 架构** - 新增 `IBoundXxxFetcher` 接口，支持绑定 Cookie 后无需重复传递
...
| v5 | v6 |
|----|-----|
| `/api/douyin/fetch_one_work` | `/api/douyin/works/video/:aweme_id` |
| `/api/douyin/fetch_work_comments` | `/api/douyin/works/:aweme_id/comments` |
| `/api/douyin/fetch_user_info` | `/api/douyin/users/:sec_uid/profile` |

#### B站

| v5 | v6 |
|----|-----|
| `/api/bilibili/fetch_one_video` | `/api/bilibili/videos/:bvid/info` |
| `/api/bilibili/fetch_video_playurl` | `/api/bilibili/videos/:avid/stream` |
| `/api/bilibili/fetch_work_comments` | `/api/bilibili/comments/:oid` |
