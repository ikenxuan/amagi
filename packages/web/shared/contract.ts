...
export interface RecordOutcome {
  ...
  /**
   * 这一发的**形状序号**（产物文件名里那个 `_V<n>`），由人在「类型产出」栏选。
   *
   * `0` = 合并进现有类型（常态）；`1` 及以上 = 单独建一个新形状，不与 `_V0` 合并。
   * 缺省表示旧 server / 旧结果，前端按 `0` 处理。
   */
  shapeIndex?: number
  /**
   * 选「单独建新形状」会落到哪个序号 —— **server 算的，前端原样送回来。**
   *
   * 界面上那两档只表达意图（合并 / 分开），而「分开」到底是 `_V1` 还是 `_V7` 取决于
   * **盘上有什么**，只有 server 读得到 corpus。前端写死 1 的后果是：这个端点已经有一份
   * `_V1` 时，选「单独建新形状」会静默合并进那一份，而界面上写着「新形状」。
   *
   * 定义是「同方向、且会与这一发共存的样本里，最小的那个空序号（≥ 1）」：
   * 空出来的序号先填（联合里不留洞）、成功与错误各算一套（`plan.ts` 里那两个 Map 是两套
   * 命名空间）、**被这一发覆盖的那份不算已占用**（否则重选一次序号就爬一格）。
   */
