...
主循环三步：**选端点 → 填参数发送 → 同屏看响应体与它的类型声明**。
决定留下就顺手写进 corpus，参数与结论记进**请求集合**（那个文件进 git），响应样本只留在本地。
...
┌─────────┬────────────┬──────────────────┬──────────────────┐
│ 端点     │ 请求        │ 响应              │ 类型              │
│ （搜索、 │ 参数 / 集合 │ 200 · 312ms · 9KB │ 本次 / 已提交 /   │
│  分组）  │ ───────    │ ───────────      │ diff / 对比       │
│         │ 参数表单    │ 脱敏后的 JSON     │ 这一份响应的      │
│ 最近     │ [发送]     │ [留下] [丢掉]     │ TypeScript 声明   │
└─────────┴────────────┴──────────────────┴──────────────────┘
```

**页面本身不滚，四块各自滚**（`lg` 以上；判据全在 `src/lib/pane.ts`）。这不是审美：
上一版是「一列卡片往下堆」，批量录 24 组之后结果区有几十屏高，而人一次只看一份 ——
「往下滚很久滚不到底」是那个版面的必然结果，不是没调好。同时横向空间几乎全空着，
而「这段 JSON 对应的类型对不对」这个问题恰恰需要两块并排才答得了。

「留下 / 丢掉 / 批量 / 生成 / 对比 / 集合」**一个都没删**，它们退到了各自那一栏的标题行或 tab 里：
那是这个工具的第二层（这份样本要不要进 corpus），而它原先与第一层（打一发看看）混在同一列，
于是最常做的事和偶尔做的事一样显眼。

窄屏（`lg` 以下）上三栏叠成三行、页面照常滚 —— 在那个宽度上锁死视口高度会让每栏只剩几行可见。
...
「类型」那一栏的四页说的是四件不同的事，别混：

- **本次** —— 刚打回来的这一份响应**单独**跑一次生成器得到的声明（`RecordOutcome.typeSource`，
  `server/declare.ts`）。它就是「这段 JSON 的类型是什么」这个问题的直接答案。
- **已提交** —— `packages/response-types/` 里**当前提交**的那一份（`GET /api/generated`）。
  它回答「我是不是在重复劳动」。**它与「本次」不可直接比**：这一份由这个端点的全部样本
  **合并**而来，比单份样本严不了 —— 出现过的键全是必需、空数组是 `unknown[]`、
  只见过 `string` 就不会有 `| null`。那句话在这一页顶上的 tooltip 里。
- **diff** —— 留下这份样本会让产物文件**变成什么样**。一个已经有类型的端点上它可能只有两行
  溯源注释，那正是「这份没带来新形状」的意思。
- **对比** —— 同一个端点两组参数各自的形状差在哪。
...
| `POST /api/record`       | 发一次请求 → 入库判定 + 脱敏统计 + 类型 diff + 高亮好的响应（`payloadHighlight`）+ **这一发单独生成的类型声明**（`typeSource`，出问题时是 `typeIssue`）+ HTTP 收据（`http`：状态码 / 墙上耗时 / 脱敏后字节数，失败那条路上也回） |
...
- **`/api/record` 回的「类型声明」与「类型 diff」不是一回事。** `typeSource` 是**这一份响应
  单独**跑一次生成器（`server/declare.ts`，纯函数、不读盘、不落盘）—— 界面「类型」栏第一页。
  `diff` 是「留下这份样本会让产物文件变成什么样」。一个已经有类型的端点上后者可能只有两行
  溯源注释，而前者仍然是一整份可以直接读、直接抄走的声明。生成失败时回 `typeIssue`
  而不是静默少一个字段 —— 那一页会把失败原因显示出来。
...
server/     Node 侧。依赖 core（注册表 + 执行管线）与 typegen（纯函数生成）
  ...
  declare.ts    一份响应 → 它的 TypeScript 声明（纯函数，有测试）。「类型」栏第一页就是它
...
src/        浏览器侧。Vite + React + Tailwind CSS v4 + @heroui/react
  App.tsx                    版面（三栏 + 左栏导航）+ 动作编排
  lib/pane.ts                「一块面板」的唯一定义：每栏自己滚、页面不滚的那几个 class
  ...
  components/HistoryList      左栏底下的「最近」：发过的每一发一行，选哪一行决定右边三栏
  components/RequestPane      「请求」栏：参数表单 / 请求集合两页 + 发送
  components/ResponsePane     「响应」栏：判定 + HTTP 收据 + 响应 JSON + 留下 / 丢掉
  components/TypePane         「类型」栏：本次 / 已提交 / diff / 对比 四页
  components/ParamForm        由 JSON Schema 派生的表单（逐字段错误；数字只给有界的上步进器）
  components/Result           上面两栏共用的块：响应 JSON、类型 diff、两条复制、「记参数」表单
