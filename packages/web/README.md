...
主循环三步：**选端点 → 填参数发送 → 在「结果」栏里看响应、在「样本处理」栏决定留不留**。
决定留下就顺手写进 corpus，参数与结论记进**请求集合**（那个文件进 git），响应样本只留在本地。

## 版面：请求 40% / 右侧 60%，右侧再分响应 70% / 样本处理 30%

```
┌─────────┬──────────────────┬──────────────────────────────────────┐
│ 端点     │ 请求 40%          │ 响应 70%                             │
│ （搜索、 │ [发送][重置][连录] │ [响应][声明][结构][diff·N] 收据[生成][仓库▸]│
│  分组）  │ 参数表单          │ ──────────────────────────────       │
│         │ [集合 5]          │ [原始|样本] 响应 JSON（四个 tab 各自滚）│
│ 最近     │                  ├──────────────────────────────────────┤
│         │                  │ 样本处理 30%                          │
│         │                  │ 判定摘要 [只保存样本][丢掉][复制]       │
└─────────┴──────────────────┴──────────────────────────────────────┘
...
**页面本身不滚，三块各自滚**（`lg` 以上；判据全在 `src/lib/pane.ts`）。这不是审美：
上一版是「一列卡片往下堆」，批量录 24 组之后结果区有几十屏高，而人一次只看一份 ——
「往下滚很久滚不到底」是那个版面的必然结果，不是没调好。同时横向空间几乎全空着，
而「换一组参数再打一发」恰恰要表单与结果并排才顺。

主循环被明确成「发送请求 → 检查响应 → 处理样本」三步，**每一步有自己可拖拽的空间**：
两处分隔条都可拖、尺寸分别持久化（`amagi-panes-horizontal` 与 `amagi-result-stack-vertical`），
用户拖过的比例优先于默认值。样本处理区**未发送时也占着那 30%**（一句空态），首发之后版面不跳。

为什么样本处理要独立成栏：它原先被压成「结果」栏底下的一条动作带，长诊断（一屏脱敏告警）
会挤占响应的阅读空间，也不能单独调高。现在它是一格完整面板：能保存时给两条互斥路径
（`只保存样本` 一键、`保存并共享参数` 只填一句中文说明 —— 身份是 server 算的参数哈希，
没有英文名可填）；不能保存时**常驻只有摘要**（结论、平台状态），讲原理的一律进 tooltip。

「保存 / 丢掉 / 批量 / 生成 / 对比 / 集合」**一个都没删**，它们退到了各自的位置：「这份样本」
的动作（保存 / 丢掉 / 复制 / 共享参数）在「样本处理」栏，仓库的两页（已提交 / 对比）在
标题行「仓库」开的抽屉里，端点级的动作（批量 / 集合）在请求栏，「生成」贴着它的产物 ——
结果栏标题行的类型声明旁边。那是这个工具的第二层（这份样本要不要进 corpus），
而它原先与第一层（打一发看看）混在同一列，于是最常做的事和偶尔做的事一样显眼。

窄屏（`lg` 以下）上三块叠成三行、页面照常滚 —— 在那个宽度上锁死视口高度会让每栏只剩几行可见。
...
- **diff**（tab）—— 留下这份样本会让产物文件**变成什么样**。它与「生成类型」读**同一份**
  `.doc.json`（`discriminantPath` 决定文件布局）—— 两条路不读同一份的后果实测过：
  界面上显示一棵判别联合目录树，而生成出来的是单类型。默认是**字段变更列表**
...
- **`/api/record` 的 `payload` 是样本，不是原始响应。** 它是「先裁剪（`trimSample`，每个数组
  留前 3 条 + 形状没见过的条目）再脱敏」之后的那一份 —— 入库的就是它，界面上「响应」页的
  「样本」档显示的也是它。全量原始响应在 `rawPayload`（未裁剪、未脱敏），「响应」页**默认
  显示原始、可切样本**；有裁剪时（`payloadTrimmed`）「样本处理」栏多一枚「已截断」Chip（如
  `emoji_list 371→3`），收据把真实响应与样本两个体积并排报出来。复制按钮跟着当前视图走，
  标签说清复制的是「原始」还是「样本（裁剪 + 脱敏后）」。缺 `rawPayload` 的那几条路
  （旧 server、`compute`、一发都没打出去）回落样本视图，切换不渲。
...
- **`/api/store` 现在做两件事，但只做你显式选的那件。** `mode: 'sample-only'` 只写样本 ——
  那是正常完成，不是失败（静默跳过会让人以为参数已经进 git）；`mode: 'sample-and-params'`
  先写样本、后按 `paramsHash` upsert 集合条目：`sampleHash` 是指向样本的指针，
  反过来的顺序会造出指向不存在文件的指针。追加没成时**待定条目留着**，
  因为样本已经落盘，参数改一处只该花人一次点击。
- **读集合也走 POST**（`op: 'list'`），不新加 GET —— 上面那个「GET 在 POST 判断之前」的既有
  顺序这一轮不碰。代价只是多两道闸（`Origin`、`Content-Type`），而这条路只有界面在走。
- **状态码分两档，这个区分是有用的**：`400` = 改你的输入（凭证命中、mode / label / `paramsHash`
  不合法、`verdict` 不认识、两个下拉框选了同一份样本）；`409` = 先去修盘上那个文件
  （集合是坏 JSON、有坏条目，或 v1 迁移冲突）。后者不是调用方的错，修法也完全不同。
  `404` 留给「没有这个端点 / 没有这份样本」。
...
src/        浏览器侧。Vite + React + Tailwind CSS v4 + @heroui/react
  ...
  components/RequestTable     这个端点的请求集合。身份是参数哈希、名字是说明（撞名带哈希消歧）；四档 verdict 同一等，删除要确认
...
- **进 git，值是真值。** 于是只放公开内容：公开作品、公开账号、搜索关键词 ——
  这也正是这个工具的全部请求参数的形状（视频 ID 这类互联网上直接搜得到的东西）。
...
界面上也有两条清楚分开的路：「样本处理」栏点「只保存样本」只把响应样本落在本地
（`sample-only`）；展开同栏的「保存并共享参数」、只填一句中文说明（`sample-and-params`），
则由 `/api/store` 在写样本之后把当前参数追加进这个集合。**身份是参数哈希，不需要人填**：
server 从待定样本的真值参数算 `paramsHash`，同参数幂等 —— 同哈希会整条替换；
凭证键命中或盘上集合有问题时，样本照常写入、集合一个字节都不动，并保留待定条目供修正后重试。
...
- **幂等按 `paramsHash`**：同哈希已存在就整条替换、不追加第二条（说明可以重复，身份不依赖它）。
  v1 文件（人填英文 `id` 的那一版）在读取时自动迁移：按每条的 `params` 重算哈希；
  两条 v1 记录算出同一个哈希是**迁移冲突**，拒绝静默覆盖，要人手工选保留哪条。
  任一次成功写操作都以 version 2 原子重写整个文件。
- **坏条目分两档**：`paramsHash` / `label` / `params` / `recordedAt` / `verdict` 错了整条不收
  （哈希会被重算，对不上的不收 —— 机器身份不信客户端）；`sampleHash` / `shapeKey` / `note`
  只丢那个字段、条目留着 —— 这个文件最有价值的部分是「什么参数 + 什么结论」，
  为一个多余的哈希把那句结论整条丢掉是本末倒置。
- **`shapeKey` 由 server 算，客户端给的一律忽略**（`server/index.ts` 的 upsert 段）。它是「这组参数
  ...
  当场算不出来时会沿用盘上同 `paramsHash` 那条的值，前提是它还指着同一份样本：**样本不进 git 而集合
