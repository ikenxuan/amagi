...
┌─────────┬──────────────────┬──────────────────────────────────────┐
│ 端点     │ 请求              │ 结果                                  │
│ （搜索、 │ [发送] [重置] [连录] │ [响应][声明][结构][diff·N] 200·312ms [生成][仓库 ▸]│
│  分组）  │ 参数表单          │ ──────────────────────────────       │
│         │                  │ [原始|样本] 响应 JSON（四个 tab 各自滚）│
│ 最近     │ [集合 5]          │ [ok][已截断][新形状] 只留样本 丢掉 复制…│
└─────────┴──────────────────┴──────────────────────────────────────┘
```
...
「只留样本 / 丢掉 / 批量 / 生成 / 对比 / 集合」**一个都没删**，它们退到了各自的位置：「这份样本」
的动作（只留样本 / 丢掉 / 复制 / 参数记进 git）在结果栏底部的动作条，仓库的两页（已提交 / 对比）在
标题行「仓库」开的抽屉里，端点级的动作（批量 / 集合）在请求栏，「生成」贴着它的产物 —— 结果栏标题行的
类型声明旁边。那是这个工具的
第二层（这份样本要不要进 corpus），而它原先与第一层（打一发看看）混在同一列，于是
最常做的事和偶尔做的事一样显眼。
...
- **`/api/record` 的 `payload` 是样本，不是原始响应。** 它是「先裁剪（`trimSample`，每个数组
  ...
  显示原始、可切样本**；有裁剪时（`payloadTrimmed`）动作条上多一枚「已截断」Chip（如
...
src/        浏览器侧。Vite + React + Tailwind CSS v4 + @heroui/react
  ...
  components/ResultPane       「结果」栏：响应（原始 / 样本两档，默认原始）/ 声明 / 结构 / diff 四个 tab + 仓库抽屉入口
  components/ResultActions    结果栏底部动作条：判定/脱敏/已截断/新形状 + 只留样本 / 丢掉 / 复制（跟当前视图）/ 参数记进 git
  components/RepoDrawer       「仓库」抽屉：已提交 / 对比 两页（说的不是这一发）
  components/ParamForm        由 JSON Schema 派生的表单（逐字段错误；数字只给有界的上步进器）
  components/Result           「结果」栏共用的块：响应 JSON、类型 diff、两条复制、「把参数也记进 git」表单
...
界面上也有两条清楚分开的路：点「只留样本」只把响应样本落在本地；展开结果栏底部的
「把这组参数也记进 git」，填英文名与一句话说明后提交，则由 `POST /api/store` 在写样本之后
把当前参数追加进这个集合。服务端按 `id` 幂等：同 id 会整条替换；凭证键命中或盘上集合有问题时，
样本照常写入、集合一个字节都不动，并保留待定条目供修正后重试。
