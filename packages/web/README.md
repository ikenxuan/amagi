...
主循环三步：**选端点 → 填参数发送 → 在「结果」栏里看响应、它的类型声明与 diff**。
...
而「换一组参数再打一发」恰恰要表单与结果并排才顺。

为什么是两栏：三栏时代的第三栏四页里只有「本次」说的是这一发，其余说的是仓库；而请求栏
22rem 装不下参数多的端点。于是「响应」与「类型」并成「结果」栏（四个 tab 都只说这一发）、
仓库那两页进了「仓库」抽屉、请求栏加宽到 28rem。**代价明说：响应与类型声明不再同屏** ——
一次只见一页。

「留下 / 丢掉 / 批量 / 生成 / 对比 / 集合」**一个都没删**，它们退到了动作条与「仓库」抽屉里：
那是这个工具的第二层（这份样本要不要进 corpus），而它原先与第一层（打一发看看）混在同一列，
于是最常做的事和偶尔做的事一样显眼。

窄屏（`lg` 以下）上两栏叠成两行、页面照常滚 —— 在那个宽度上锁死视口高度会让每栏只剩几行可见。
...
「结果」栏的四个 tab 与「仓库」抽屉说的是四件不同的事，别混：

- **声明**（tab）—— 刚打回来的这一份响应**单独**跑一次生成器得到的声明（`RecordOutcome.typeSource`，
  `server/declare.ts`）。它就是「这段 JSON 的类型是什么」这个问题的直接答案。
- **已提交**（「仓库」抽屉）—— `packages/response-types/` 里**当前提交**的那一份（`GET /api/generated`）。
...
- **`/api/record` 回的「类型声明」与「类型 diff」不是一回事。** `typeSource` 是**这一份响应
  单独**跑一次生成器（`server/declare.ts`，纯函数、不读盘、不落盘）—— 界面「结果」栏的「声明」tab。
...
src/        浏览器侧。Vite + React + Tailwind CSS v4 + @heroui/react
  ...
  components/RequestPane      「请求」栏：参数表单 + 集合抽屉 + 发送
