 ...
 * ## 为什么这一件事非得进 JS
 *
 * 版面的其余部分一直是纯 CSS 断点（`lg:flex-row`、`2xl:grid-cols-…`），那样最好：
 ...
 *   而「上下堆」的旧毛病不是上下堆本身，是页面无限长滚不到底 —— 那由锁死视口高度解决。
...
/**
 * 三档宽度各自的媒体查询。**取值必须与 Tailwind 的 `lg` / `2xl` 逐字相同** ——
 * 版面里还有别的东西按那两个断点走（`<main>` 的 `lg:h-screen`），两处错开会出现
 * 「锁了高度但没并排」这种半截状态。Tailwind v4 的默认值是 `64rem` 与 `96rem`。
 */
const ROOMY = '(min-width: 64rem)'
const WIDE = '(min-width: 96rem)'
