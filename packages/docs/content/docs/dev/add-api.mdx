...
declare function fetchBilibiliInternal(methodType: string, params: Record<string, any>, options: { cookie?: string }): Promise<Result<any>>
 ...
 */
export async function fetchVideoTags(options: BilibiliVideoTagsOptions, cookie?: string): Promise<Result<any>> {
  return fetchBilibiliInternal('videoTags', { avid: options.avid }, { cookie })
}
```
...
// ---cut---
async function handleWbiMethod(data: { methodType: string; param: string }, cookie: string) {
...
// ---cut---
async function handleDouyinMethod(baseUrl: string, userAgent: string, methodType: string) {
  // 生成 a_bogus 签名
...
// ---cut---
async function handleXiaohongshuMethod(path: string, a1Cookie: string, methodType: string) {
