...
// 方式 1：通过客户端实例
const client = amagi({ cookies: { douyin: 'your_cookie' } })
...
```typescript twoslash
import { amagiEvents, type LogEventData } from '@ikenxuan/amagi'
// ---cut---
const handler = (data: LogEventData) => console.log(data.message)
