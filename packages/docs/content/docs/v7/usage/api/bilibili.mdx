...
<Callout type="info">本页是 **SDK 方法**参考（27 个）。同一批端点的 **HTTP 形态**（路径、query 参数、响应信封、在线调用）见 [HTTP 端点参考](/docs/v7/usage/api/http) —— 那部分由 `packages/core/openapi.json` 生成，不手写。</Callout>
