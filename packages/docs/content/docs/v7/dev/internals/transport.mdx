...
`PLATFORM_RUNTIME` 里 `kuaishou: {}` —— **既没有签名器，也没有默认 judge**。而仓库里确实存在一个完整的 `KuaishouSigner`（能产出 `__NS_hxfalcon`、`caver`、`kww` 头）和一个 `kuaishouJudge`，两者在生产代码里**零调用点**，只被测试引用。

具体后果：

| 本该发生 | 实际发生 |
| -------- | -------- |
| 请求带 `__NS_hxfalcon` 签名与 `kww` 头 | 都不带。端点把 `signPath` 放进 spec，但 `signPath` 在构造 axios 配置时被丢弃 |
| 按快手 judge 判定（`result === 1` 才算成功） | 走 `execute` 的兜底判定「HTTP 2xx 即成功」 |

`PLATFORM_RUNTIME` 的注释写「URL 由 api.ts 预签名」，但 `api.ts` 只拼 query、不写 `__NS_hxfalcon`。快手某个端点的 JSDoc 还写着「响应判定用快手 judge」，与装配现状矛盾。

<Callout type="warn">
  改快手相关代码前先确认这是有意还是断链 —— 两种可能都存在（快手的 graphql 与 live_api
  也许确实不需要 amagi 侧签名）。**不要**因为「注释说有 judge」就假定 `result !== 1` 会被判失败。
</Callout>
