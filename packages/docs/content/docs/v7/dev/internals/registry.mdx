...
| `sign` | `TSign \| false \| SignFn` | 字符串查平台签名器表（`TSign` 默认宽 `string`，平台的 `defineXxxEndpoint` 把它收窄到签名器名联合、写错编译期即报错）；`false` 显式不签；函数即一次性签名器 |
| `response` | `type<T>()` | `TData` 的**唯一**来源。`normalize` / `paginate.merge` 的返回包 `NoInfer<TData>`、只被检查不参与推导；只有 `compute` 参与推导（可只写 `compute` 不写 `response`） |
...
| 槽位 | 端点数 | 做什么 |
| ---- | ------ | ------ |
| `retryOn` | 42（24 在 B站、17 在抖音） | 命中这些错误码时端点级重试。B站的 `-412` → `RISK_CONTROL`；快手弹幕约 13% 概率的 `result: 11` → `PLATFORM_UNAVAILABLE`；抖音的 Argus 拦截 → `ANTIBOT_PAGE` |
| `retryFresh` | 17（全在抖音） | 重试时重新 build + 重新签名而不是原样重放。Argus 按单次请求的 token 组判定，重放同一个 `msToken` + `a_bogus` 必然同样被拦 |
...
| `aggregate` | 2 | 「同构分段并发」的带类型收尾（弹幕按时间区间切段并发），取代这类端点的 `normalize`；与 `normalize` **互斥**（同时写编译报错，见[契约与信封](/docs/v7/dev/internals/contracts)）。抖音 / 快手弹幕 |
| `normalize` | 1 | 把响应整理成最终 data 的**通用兜底**（第一参 `unknown`）。分页用 `paginate.merge`、同构分段用 `aggregate`，剩下的才用它。当前仅快手 `userProfile`（异构聚合） |
| `compute` | 2 | 纯本地计算，不发请求（B站 AV/BV 互转） |
| `sign: false` | 4 | 显式不签名（抖音四条免鉴权端点）。签名端点改用 `SignStep` 清单，如 `[msToken(184), aBogus(), secsdk()]`，按 phase 排序执行（见 `sign/steps.ts`） |
...
当前规模 —— 抖音 23、B站 27、快手 8、小红书 7，**合计 65**。这个数字在七个地方被断言，改了端点数就得同步（见[新增接口](/docs/v7/dev/add-api)）。
...
```mermaid
flowchart LR
  D["endpoints/*.ts<br/>65 份 defineEndpoint 声明"]
...
| OpenAPI 规范 | `server/openapi.ts` → `scripts/gen-openapi.mts` | 四个 registry | **提交**（`packages/core/openapi.json`，65 条 path） |
| 自托管 `/openapi.json` | `server/auth.ts` 的 `mountOpenApiSpec` | 同上，**现算** | 无 |
| 65 个 HTTP 端点页 | `packages/docs/scripts/generate-docs.ts` | `openapi.json` | gitignore |
...
`client/method-names.ts` 有 65 条 `'<平台>.<短名>' → v6 方法名` 的映射。它存在是因为有 16 处不规则命名，拼不出来：
...
- **HTTP 65 页** —— `fumadocs-openapi` 的 `generateFiles`，输入是 `openapi.json`，`per: 'operation'` + `groupBy: 'tag'`。`beforeWrite` 钩子补中文标题、平台图标，并追加一份自拼的索引页。
...
## `platforms/` 一层，与它底下的 `legacy/` 一角

`packages/core/src/platforms/` 是**唯一**的平台实现目录（2026-09-14 之前这里并列着 `platform/` 单数与
`platforms/` 复数两个目录，读仓库极易走错；合并后 `platform/` 这个名字不再存在）。它内部分成两片：

| | `platforms/<平台>/` | `platforms/legacy/` |
| --- | --- | --- |
| 规模 | 128 文件 / 12548 行 | 13 文件 / 2543 行 |
| 是什么 | **v7 实现层** | **v6 保留实现**（公开面照旧，行为逐字不动） |
| 内容 | `endpoints/`（65 份声明 + registry）、`api.ts`（URL 构造纯函数）、`config.ts`、`judge.ts`、`sign/`、`session/`、`decode/`、`assemble/`、`passport/`（仅 douyin）、`routes.ts`、`utils.ts` | 四个 v6 版 `API.ts`、v6 的 `wbi.ts` / `qtparam.ts` / `douyin/sign`、`kuaishou/sign`（分叉的 3 个文件）、`xiaohongshu/sign`、`searchTypes.ts` |
| 在公开导出面上？ | `routes.ts` 与 `utils.ts` 在，其余不在 | 在（签名器经 `utils.ts` 转出；`xxxApiUrls` 由 `src/index.ts` 显式点名导出） |
| 新增端点要碰吗 | 要 | 不要 |

### `<平台>/utils.ts` 是公开面，`routes.ts` 是唯一的路由挂载路径

四个 `createXxxRoutes(cookie, requestConfig?)` 住在 `platforms/<平台>/routes.ts`，是**唯一**的路由挂载
路径（门面 `startServer` 只认它们），内部完全走 v7：`createRoutes(platform, registry, makeClientCtx(...))`。

`utils.ts` 是每平台的公开工具面。它把三类东西凑到一起，**不复制任何实现**：

- `createXxxRoutes` —— 同目录的 `./routes`
- v7 实现（`av2bv` / `parseDmSegMobileReply` / `douyinPassport`）—— 直接引同平台的 v7 模块
- v6 实现（`wbi_sign` / `qtparam` / `xxxSign`）—— 引 `../legacy/<平台>/`

v6 的 `xxxApiUrls` **不经 utils** —— 它由 `src/index.ts` 从 `legacy/<平台>/API.ts` 显式点名导出，
这样 v7 门面上只有 v7 自己的构造器（`client.<平台>.apiUrls`），两份同名不同义的东西不坐在同一个对象上。

`src/index.ts` **不写** `export * from './platforms'`：那是实现层，整体 star 出去会把签名器、判定、
端点声明这些内部构件一并泄漏到公开面。改成四个 `export * from './platforms/<平台>/utils'` 逐个点名
—— 公开什么，看 `utils.ts` 一个文件就知道。

### legacy 与 v7 是两份代码，不是一份的两个入口

包顶层导出的 `douyinApiUrls` 这类 v6 构造器（`legacy/<平台>/API.ts`），与 SDK 内部实际发请求用的
URL 构造器（`<平台>/api.ts`，挂上门面的是 `client.<平台>.apiUrls`）**是两份代码**，两边行数不同。
要改 URL 行为时必须确认自己改的是哪一份。

签名同理：`legacy/douyin/sign`、`legacy/kuaishou/sign`、`legacy/xiaohongshu/sign` 与 v7 的同名模块是
**分叉的两份**。只有算法叶子（`a_bogus` / `x_bogus` / `he` / `hudr` / `primitives` / `bv2av` /
`danmaku` / `config`）在合并时去重成了一份 —— 它们当时逐字节相同，v6 侧改为直接引 v7，少了 8 个副本。

分叉的那几个要留神：`test/platforms/<平台>/sign.test.ts` 一类用例拿 **v6 那份当 oracle**，对同一入参
断言 v7 输出与 v6 完全相等。这层对照依赖两份代码同时存在，动任何一侧前先读那些用例。

<Callout type="info">
  从 `src/index.ts` 递归跟随 import，`platforms/` 一层共 141 个 `.ts`，**141 个全部可达**。
