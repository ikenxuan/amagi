...
当前规模 —— 抖音 19、B站 27、快手 6、小红书 7，**合计 59**。这个数字在五个地方被断言，改了端点数就得同步（见[新增接口](/docs/v7/dev/add-api)）。
...
```mermaid
flowchart LR
  D["endpoints/*.ts<br/>59 份 defineEndpoint 声明"]
...
| OpenAPI 规范 | `server/openapi.ts` → `scripts/gen-openapi.mts` | 四个 registry | **提交**（`packages/core/openapi.json`，59 条 path） |
| 自托管 `/openapi.json` | `server/auth.ts` 的 `mountOpenApiSpec` | 同上，**现算** | 无 |
| 59 个 HTTP 端点页 | `packages/docs/scripts/generate-docs.ts` | `openapi.json` | gitignore |
...
`client/method-names.ts` 有 59 条 `'<平台>.<短名>' → v6 方法名` 的映射。它存在是因为 v6 的命名有 15 处不规则，拼不出来：
...
其余 44 条按 `fetch` + 短名首字母大写的规则派生，运行时与类型层都有同款兜底。
...
<Callout type="warn">
  从 `src/index.ts` 递归跟随 import，两个平台目录共 138 个 `.ts` 里有 131 个可达。**7 个不可达的文件全在
  v7 目录**：`platforms/kuaishou/judge.ts` 与 `platforms/kuaishou/sign/` 下 6 个。原因是
  `client/runtime.ts` 的 `PLATFORM_RUNTIME` 里 `kuaishou: {}` —— 既没装签名器也没装默认 judge。
  后果见[传输与签名](/docs/v7/dev/internals/transport)。
</Callout>

## `doc` 字段为什么被测试而不是类型强制

`doc` 在 `EndpointDef` 上是可选的，这样 59 个现存端点一个不改也能编译。真正的强制在 `test/contracts/endpoint-doc.test.ts`：拿四个注册表逐个断言 `doc.summary` 非空、≤40 字、不以句号结尾、同平台内不重复。
