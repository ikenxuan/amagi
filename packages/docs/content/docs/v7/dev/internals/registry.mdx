...
| 在公开导出面上？ | `routes.ts` 与 `utils.ts` 在，其余不在 | 在（经 `utils.ts` 转出） |
| 新增端点要碰吗 | 要 | 不要 |
...
- v6 实现（`xxxApiUrls` / `wbi_sign` / `qtparam` / `xxxSign`）—— 引 `../legacy/<平台>/`
...
`amagi.douyin.douyinApiUrls` 这类工具集，与 SDK 内部实际发请求用的 URL 构造器（`<平台>/api.ts`）
**是两份代码**，两边行数不同。要改 URL 行为时必须确认自己改的是哪一份。
