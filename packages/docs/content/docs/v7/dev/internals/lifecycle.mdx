...
flowchart TD
  ...
  RT{"命中 retryOn？"}
  NM["⑧ normalize / paginate.merge（可选）"]
...
10. **`normalize` / `paginate.merge`** —— 把响应裁剪 / 整形为声明的返回类型。非分页端点用 `normalize`（多分片时它收到的是数组）；分页端点用 `paginate.merge`，它收到收窄好的 `PaginatedValue`（`lastPage` 加跨页累积并截断的 `items`）。
