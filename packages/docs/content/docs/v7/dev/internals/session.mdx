...
- **`resolveTarget`** 对非有限数回退到 `maxPageSize`，负数归 0，其余 `Math.trunc`。`number: 0` 意味着**一个请求都不发** —— 结果落 `PaginateOutcome` 的 `empty: true` 判别支，没有 `lastPage` 可言。
...
2. **收尾全部写在 `paginate.merge` 里，解构 `{ lastPage, items }`** —— `merge` 的第一参是收窄好的 `PaginatedValue<TPage, TItem>`：`lastPage` 恒有值（空跑由 `empty` 判别支单独承载），`items` 是跨页累积并截断到目标条数的条目。`PaginatedValue` **不带 `pages`**（旧结构留每页完整解码体、没人读，纯内存滞留，已删）。
3. **全部没有声明 `partial`** —— 翻页分支里 `partial` 本来也被忽略。

`merge` 的产出形状统一是「最后一页的浅拷贝 + 跨页聚合后的 items 塞回列表字段」（`...lastPage` 直接展开，因 `lastPage` 恒有值、字段保持必填，不必再 `?? {}` 兜底又断言回去）。所以**调用方看到的不是分页结构，而是平台原始形状**：
...
<Callout type="warn">
  **`watch()` 返回的不是 `AmagiResult`。** 它是 `{ ok: true, credential } | { ok: false, error }` ——
  没有 `success` / `data` / `message` / `meta`。早期设计稿里写的
