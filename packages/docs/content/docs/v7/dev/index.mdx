...
| 文档站（`packages/docs/`） | `pnpm --filter=docs run typecheck` | 它串了 `<include>` 区段、twoslash 示例、侧边栏三道静态门，都在整站构建之前 |
...
| `pnpm build:docs` | 文档站整站构建（twoslash 求值 + 死链检查） |

`build:docs` 要把全站上百个代码块真编译一遍，CI 那台 4 vCPU / 16 GiB 的 runner 上峰值贴着上限
（workflow 里专门加了 swap，`next.config.mjs` 把 `cpus` 压到 2）。**本机跑不动就交给 CI** ——
上面表里那条 `pnpm --filter=docs run typecheck` 已经把最常坏的两种挡在前面了：`<include>` 引的区段被改名、
示例编译不过。它们只要有 TypeScript 就能跑，不需要 Next 的 bundler。
