...
| `pnpm build` | 构建所有包 |
| `pnpm typecheck` | 三个包的类型检查（docs 那份还串了三道静态门） |
...
提交前最少跑 `typecheck` + `lint` + `test`。动了端点声明还要 `openapi:check`，动了文档要 `build:docs` —— 这两条都是 CI 的必需检查。命令的完整展开与 CI 各道门的判定条件见[仓库工程](/docs/v7/dev/internals/repo)。
