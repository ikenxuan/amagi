...
`validation/`、`types/`、`utils/` 等旧目录与 v6 低阶入口（`transport/legacy.ts` 等
@deprecated 保留点），正按阶段 6 的删除清单清理。v6 的平台实现已收进
`platforms/legacy/`（原先并列的 `platform/` 单数目录已于 2026-09-14 合并掉）。
...
flowchart TD
  ...
  P8["⑧ normalize / aggregate / paginate.merge（可选）<br/>整形为声明的返回类型"]
  R["AmagiResult 信封"]
  F --> P1
  P1 -->|校验失败，不发请求| R
  P1 -->|通过| P2
...
最后由 `paginate.merge` 把跨页累积的条目收敛成一次结果（分页端点的收尾在这里，不在端点级 `normalize`）。整段调用从头到尾只有一个 `requestId`，实际发出几个请求看 `meta.attempts`。
...
真实声明里用的是平台专属的 `defineDouyinEndpoint`（就是 `defineEndpoint` 绑定了抖音签名器名联合，把 `sign` 收窄成 `'a-bogus' | 'x-bogus'`、写错编译期即报错，见[新增接口](/docs/v7/dev/add-api)）；它和 `defineEndpoint` 运行时都是恒等函数，全部价值在类型推导：`TParams` 从 `params` 推出，`TData` 从 `response` / `normalize` / `compute` 推出。响应类型**直接复用 v6 的 `types/ReturnDataType` 快照类型**（映射键与端点短名一一对应），v6 快照自带顶层索引签名，平台加字段不算 breaking。
...
| `sign`      | 签名器名（平台专属 `defineXxxEndpoint` 把它收窄到本平台签名器名联合，写错编译期即报错）/ `false`（显式不签名）/ 一次性函数 |
| `decode`    | 响应不是普通 JSON：protobuf / multi-JSON / HTML |
| `paginate`  | 声明式翻页：`maxPageSize` / `items` / `hasMore` / `nextParams` / `merge`（收尾）；一页与最终返回不同形时用 `page` 令牌覆盖 |
| `partial`   | 多请求部分失败语义：`'tolerate'`（缺失部分留空）/ `'fail'` |
| `judge`     | 覆盖平台默认判定 |
| `normalize` | 把响应裁剪 / 整形为最终 `data`（**非分页、非同构分段**；分页收尾用 `paginate.merge`、同构分段收尾用 `aggregate`） |
| `aggregate` | 同构分段并发（`build` 返回同形数组）的带类型收尾，取代 `normalize`；与 `normalize` **互斥** —— 同时写编译报错，另有测试门兜底 |
| `compute`   | 纯本地计算，不发请求 |
| `response`  | 响应类型令牌 `type<Foo>()` |
| `retryOn`   | 命中这些错误码时重试（如 B站 `-412` → `RISK_CONTROL`、抖音 Argus → `ANTIBOT_PAGE`） |
| `retryFresh` | 重试时重新 build + 重新签名，而不是重放同一个请求（抖音换 msToken / a_bogus 过 Argus） |
...
| 响应类型 | `response` / `normalize` / `aggregate` / `compute` 推导 |
