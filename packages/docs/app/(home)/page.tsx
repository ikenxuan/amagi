      ...
      <section className="flex flex-col items-center justify-center text-center px-4 py-24 md:py-32">
        <div className="max-w-4xl">
          ...
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight mb-6">
            <span className="bg-linear-to-r from-fd-foreground to-fd-muted-foreground bg-clip-text text-transparent">@ikenxuan/amagi</span>
          </h1>
      ...
      <section className="px-4 pb-16">
        <div className="max-w-3xl mx-auto">
          <div className="rounded-xl border border-fd-border bg-fd-card overflow-hidden">
            ...
            <div className="p-4 text-sm overflow-x-auto [&_pre]:bg-transparent! [&_code]:bg-transparent!">{html}</div>
          </div>
        </div>
      </section>
      ...
      <section className="px-4 py-16 border-t border-fd-border">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            <FeatureCard icon={<PlatformIcon />} title="多平台支持" description="抖音、B站、快手、小红书的主流数据接口，统一的调用方式" />
            <FeatureCard icon={<CodeIcon />} title="SDK + HTTP" description="支持 SDK 直接调用或启动本地 HTTP 服务，灵活选择" />
            <FeatureCard icon={<ShieldIcon />} title="参数校验" description="基于 Zod 的严格参数验证，统一的响应格式" />
            <FeatureCard icon={<TypeIcon />} title="类型安全" description="完整的 TypeScript 类型定义，支持 strict 模式" />
            <FeatureCard icon={<PackageIcon />} title="双模块输出" description="同时支持 ESM 与 CJS，兼容各种项目环境" />
            <FeatureCard icon={<EventIcon />} title="事件驱动" description="v6 全新事件系统，灵活的日志与监控" />
          </div>
        </div>
      </section>
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <div className="rounded-xl border border-fd-border p-6 transition-colors hover:bg-fd-accent/50 cursor-default">
      <div className="w-10 h-10 rounded-lg bg-fd-accent flex items-center justify-center mb-4 text-fd-foreground">{icon}</div>
...
function QuickLink({ href, title, description }: { href: string; title: string; description: string }) {
  return (
    <Link href={href} className="group rounded-xl border border-fd-border p-4 transition-colors hover:bg-fd-accent/50 cursor-pointer">
