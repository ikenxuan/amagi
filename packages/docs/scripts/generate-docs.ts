...
四个平台共 ${total} 个 HTTP 端点。本节每一页都由端点注册表生成的 OpenAPI 规范派生，
所以这里的路径、参数与响应形状不会与代码脱节。
...
<Callout type="info">
  本页 ${shorts.length} 个方法由端点注册表生成：参数表、摘要与废弃标记都取自
  端点声明。同一批端点的 **HTTP 形态**（路径、query 参数、响应信封、在线调用）见
...
// 两份派生物，都从这里出去：
//
// - `v7/changelog/meta.json` —— 侧边栏清单（板块「发布说明」+ 概述 / 版本列表两个分区）。
//   从前手写，于是「发了版忘了登记」是一条只有人眼能拦的缝：文件在、侧边栏不显示。
//   现在扫目录现生成，`scripts/release.ts` 里那道「登记进 meta」的门跟着撤掉。
// - `v7/changelog/index.mdx` —— 「所有版本」索引页。每个版本一个 `##` 标题，
//   右侧 TOC 于是自动列出全部版本 —— 卡片做不到这件事，这正是它不用卡片的原因。
//
// 版本页本身仍是手写的（一版一页，`scripts/release.ts` 的发版门盯着它），
// 这里只读它的 frontmatter 拿摘要。

const CHANGELOG_DIR = './content/docs/v7/changelog'
const CHANGELOG_URL = '/docs/v7/changelog'

/**
 * 版本号逐段比较（升序）。
 *
 * `7.0.0-beta.10` 要排在 `7.0.0-beta.9` **后面** —— 字符串比较给不出这个答案，
 * 而 beta 才刚走到个位数，这属于「今天不写、明年才炸」的坑。
 * 核心段相同时带预发布后缀的更小（`7.0.0-beta.1 < 7.0.0`），与 semver 同向。
 * @param left - 版本号
 * @param right - 版本号
 * @returns 负数 / 0 / 正数
 */
const compareVersions = (left: string, right: string): number => {
  const [coreLeft = '', preLeft] = left.split('-', 2)
  const [coreRight = '', preRight] = right.split('-', 2)
  const core = compareSegments(segmentsOf(coreLeft), segmentsOf(coreRight))
  if (core !== 0) return core
  if (preLeft === preRight) return 0
  if (preLeft === undefined) return 1
  if (preRight === undefined) return -1
  return compareSegments(segmentsOf(preLeft), segmentsOf(preRight))
}

/** `7.0.0-beta.10` → `[7, 0, 0, 'beta', 10]`：数字段按数字比，字母段按字面比 */
const segmentsOf = (version: string): (number | string)[] =>
  version.split(/[.-]/).map((segment) => (/^\d+$/.test(segment) ? Number(segment) : segment))

const compareSegments = (left: (number | string)[], right: (number | string)[]): number => {
...
/** 只认顶层「一行一个 `key: 值`」—— 与 `scripts/check-sidebar.mjs` 同一套宽容度 */
const frontmatterValue = (content: string, key: string): string => new RegExp(`^${key}:\\s*(.+)$`, 'm').exec(content)?.[1]?.trim() ?? ''

const changelogFiles = (await readdir(CHANGELOG_DIR)).filter((file) => file.endsWith('.mdx') && file !== 'index.mdx')
const changelogVersions = changelogFiles.map((file) => file.slice(0, -'.mdx'.length)).sort((a, b) => compareVersions(b, a))
if (changelogVersions.length === 0) {
  // 空输入下生成的是一副空壳侧边栏，而它看起来完全正常 —— 宁可红一次
  throw new Error(`${CHANGELOG_DIR} 里一个版本页都没有 —— 「发布说明」会生成成空壳，先补页`)
}

const changelogEntries = await Promise.all(
  changelogVersions.map(async (version) => ({
    version,
    description: frontmatterValue(await readFile(join(CHANGELOG_DIR, `${version}.mdx`), 'utf8'), 'description')
  }))
)

// 侧边栏清单。分隔符写 `---[图标]名字---`：图标不能省 —— 少一个 `check-sidebar.mjs`
// 就红一次（图标名拼错反而零报错，那个脚本正是为这两件事写的）
await writeFile(
  join(CHANGELOG_DIR, 'meta.json'),
  `${JSON.stringify(
    {
      title: '发布说明',
      ...
      icon: 'Rocket',
      root: true,
      pages: ['---[Compass]概述---', 'index', '---[Tag]版本列表---', ...changelogVersions]
    },
    null,
    2
  )}\n`,
...
// 「所有版本」页。每版一个 `##` 标题 —— 右侧 TOC 由标题生成，卡片给不了这个导航，
// 这就是这一页不用卡片的原因
const changelogIndex = `---
title: 所有版本
description: amagi v7 全部版本的发布说明 —— 新功能、修复与破坏性变更
icon: ListTree
---
...
<Callout type="tip" title="正在使用 AI 进行开发？">
  ...
  它会自动比对你当前的版本与目标版本的差异，并把必要的改动应用到代码里。
  了解更多请参阅[Agent Skills](/docs/v7/ai)。
</Callout>

${changelogEntries
  .map(({ version, description }) => `## ${version}\n\n${description}\n\n[阅读完整发布说明 →](${CHANGELOG_URL}/${version})`)
  .join('\n\n---\n\n')}
`

await writeFile(join(CHANGELOG_DIR, 'index.mdx'), changelogIndex, 'utf8')

console.log(`已生成发布说明：${CHANGELOG_DIR}（${changelogVersions.length} 个版本）`)
