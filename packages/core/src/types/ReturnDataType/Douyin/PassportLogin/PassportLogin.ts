 ...
 * 一次即可覆盖全部分支。
 *
 * **刻意豁免 `<Endpoint>/<Endpoint>_V0.ts` 的文件名约定，别「顺手统一」加 `_V0`。**
 * `_V<n>` 的语义是「同判别式下的响应形状序号」，只对实测快照成立；这份是全仓唯一一份
 * 归一化后的手写类型，形状由协议层决定而不是由服务端报文决定，标个形状序号会说谎。
 *
 * **将来的类型生成器也要豁免这份文件**：每个分支的 JSDoc、来源标注与收敛决策都是手写的
 * 语义信息，采样生成不出来；生成器一旦覆盖它，等于把归一化层的契约降级成原始报文快照。
 * 它同时是手写类型的样板 —— 需要手写归一化类型时照这份的注释密度来写。
 */
