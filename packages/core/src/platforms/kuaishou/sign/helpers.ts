...
export const createKuaishouAnonymousKwwCache = (): KuaishouAnonymousKwwCache => {
  const cache: KuaishouAnonymousKwwCache = {
    ...
    get: () => {
      if (!cache.value) {
        cache.value = `${encryptKuaishouAnonymousKwwSeed(generateKuaishouAnonymousKwwSeed())}${KUAISHOU_ANONYMOUS_KWW_SUFFIX}`
      }
      return cache.value
    }
  }
 ...
 */
export const deriveKuaishouAnonymousKww = (cache?: KuaishouAnonymousKwwCache): string => {
  if (cache) {
    return cache.get()
  }
...
export const deriveKuaishouKww = (cookie?: string, cache?: KuaishouAnonymousKwwCache): string => {
  ...
  return deriveKuaishouAnonymousKww(cache)
}
