    ...
    get: (target, prop) => {
      ...
      for (const [endpoint, def] of Object.entries(registry)) {
        if (methodNameFor(platform, endpoint) === prop) {
          ...
          // defaultEventBus 说明）。
          //
          // `error.challenge`（风控验证页地址）与平台的 `observe` 旁观者
          // （抖音回收响应头里的 webid）**三个入口都有** —— 它们经
          // `makeClientCtx` 从 `PLATFORM_RUNTIME` 装配，不受 `debug` 管。
          // 那正是它们没做成 `debug` 一部分的理由：撞验证码时「怎么过去」
          // 是必要信息，webid 更是下一次请求能不能拿到数据的前提，
          // 不该只有开了排障开关的 client 形态才拿得到。
