 ...
 *
 * 为什么需要它：`types/ReturnDataType/` 是全仓最大的一块（160 文件 / 26,258 行，
 * 占 `types/` 全目录的 97%），编译产物 `dist/*.d.ts` 已经 **806 KB**
 * （825,177 字节，含接进来的生成树）—— 那是每个下游 `tsc` 都要读一遍的东西。
 * 而响应类型自动化（见
 ...
 *   （生成产物逐个替换掉它），所以余量刻意小。**树变小之后预算必须跟着收紧** ——
 *   棘轮不往回拧就不是棘轮了：合掉那 2 个假变体 `_V1` 之后手写树从 27,249 行掉到
 *   26,258 行，而预算还留在按旧数字算的 29,500 上，余量从 +8% 悄悄变成 +12%，
 *   等于凭空多出 1,200 行可以无声无息加回来的额度。
 ...
 * 预算。**改这里要在提交信息里写清为什么。**
 *
 * 手写树留的余量是刻意小的（总量、文件数约 +10%、单文件约 +8%）：余量给得大，门禁就形同虚设。
 * 而且它是棘轮 —— 树变小时这三个数要**跟着往回拧**，见上面文件注释里那笔账。
 */
const BUDGETS: readonly TreeBudget[] = [
  {
    label: '手写',
    root: 'src/types/ReturnDataType',
    // 实测 26,258 行 / 160 文件 / 最大 2,315（`Douyin/UserLiveVideos/UserLiveVideos_V0.ts`），2026-09-05。
    // 上一版按合并 `_V1` 之前的 27,249 行 / 162 文件算，合完没跟着收紧（棘轮没往回拧）。
    totalLines: 28_900,
    fileLines: 2_500,
    fileCount: 176,
    ratchet: true
  },
  {
    ...
    // 实测 2,475 行 / 28 文件 / 最大 813（`bilibili/Comments/Comments_V0.ts`），2026-09-05。
